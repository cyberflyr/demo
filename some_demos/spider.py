...
def main():
    ...
    res = requests.get(
        "https://www.okcupid.com/1/apitun/profile/8682176196122051737/answers?filter=1",
        headers=headers,
    )
    ...
    while len(final_list) != paging["total"]:
        ...
        time.sleep(1)
        res = requests.get(
            f"https://www.okcupid.com/1/apitun/profile/8437071393684167093/answers?after={after_cursor}&filter=1",
            headers=headers,
        )
        data = json.loads(res.content).get("data")
        paging = json.loads(res.content).get("paging")

...
def get_register_info():
    register_data = (
        json.loads(request.form.get("data")) if request.form.get("data") else None
    )
    ...
    graduation_year = (
        int(register_data.get("GraduationYear"))
        if register_data.get("GraduationYear")
        else -1
    )
...
def do_search():
    ...
    university = University.query.filter(
        University.university_name.like(f"%{university_name}%")
    ).first()
    res = {}
    if university:
